...
main.py
────────
Entry point for the Generalized Execution Agent.

CORRECT USAGE — client gives instruction to planner:
    python main.py --instruction "Generate daily sales report"

DEBUG USAGE — bypass planner, test a specific client config:
    python main.py --debug-client sales_report

Flow:
    --instruction  →  planner_agent  →  execution engine  →  handlers
    --debug-client →  config.yaml    →  execution engine  →  handlers
                      (skips planner, for testing only)
"""
